...
const metaTemplates=[{id:'template-e2e',name:'retomar_atendimento',language:'pt_BR',category:'UTILITY',status:'APPROVED',components:[{type:'BODY',text:'Olá {{1}}, podemos continuar seu atendimento?'}]},{id:'template-recovery-e2e',name:'peclat_recuperacao_lead_1',language:'pt_BR',category:'MARKETING',status:'APPROVED',components:[{type:'BODY',text:'Olá, {{1}}! Tudo bem? 😊'}]}];
...
const meta=createHttpServer((request,response)=>{
 ...
 else if(request.method==='POST'&&path.endsWith('/messages')){const chunks:Buffer[]=[];request.on('data',(chunk:Buffer)=>chunks.push(chunk));request.on('end',()=>{const content=Buffer.concat(chunks).toString('utf8');if(content.includes('Falhar Meta')){response.statusCode=400;response.end(JSON.stringify({error:{code:131000,message:'Falha simulada da Meta'}}));return;}response.end(JSON.stringify({messages:[{id:`wamid.e2e.outbound.${Date.now()}`}]}));});}
...
const knowledgeUser=(await database().query("INSERT INTO users(email,name,password_hash) VALUES ('knowledge@e2e.local','Admin Base IA E2E',$1) RETURNING id",[hash])).rows[0];
await database().query("INSERT INTO memberships(organization_id,user_id,role_code) SELECT id,$1,'admin' FROM organizations WHERE slug='peclat-solar'",[knowledgeUser.id]);
