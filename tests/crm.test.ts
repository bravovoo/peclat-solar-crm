...
import {addMaintenanceItem,addTicketComment,getMaintenance,getTicket,getWarranty,linkTask,listClaims,listLinkedTasks,listMaintenanceItems,listMaintenances,listTickets,listWarranties,postSalesDashboard,postSalesOptions,saveClaim,saveMaintenance,saveTicket,saveWarranty,ticketHistory} from '../src/modules/post-sales/repository';
import {addPostSalesFile,deletePostSalesFile,getPostSalesFile,listPostSalesFiles} from '../src/modules/post-sales/files';
import {processStorageDeletionJobs} from '../src/modules/operations/maintenance';
...
test('envio por e-mail anexa o PDF e registra destinatário nos históricos',async()=>{
 ...
 const requestId=randomUUID(),payload={recipient:'CLIENTE@EXAMPLE.COM',subject:'Seu orçamento Peclat Solar',message:'Olá, segue o orçamento solicitado.',client_request_id:requestId};let sends=0,captured:Parameters<DocumentMailProvider['sendDocument']>[0]|undefined;const mail:DocumentMailProvider={async sendDocument(input){sends++;captured=input;return {messageId:'message-test-1'};}};const sent=await sendDocumentEmail(seller,document.id,payload,mail);
 ...
 const repeated=await sendDocumentEmail(seller,document.id,payload,mail);assert.equal(repeated.email.id,sent.email.id);assert.equal(sends,1);
 ...
 await assert.rejects(()=>sendDocumentEmail(otherSeller,document.id,{recipient:'x@example.com',subject:'Bloqueado',message:'Não enviar.',client_request_id:randomUUID()},mail),{status:404});await assert.rejects(()=>sendDocumentEmail(seller,document.id,{recipient:'inválido',subject:'Teste',message:'Teste',client_request_id:randomUUID()},mail));
 const retryId=randomUUID(),retryPayload={recipient:'cliente@example.com',subject:'Nova tentativa',message:'Mensagem.',client_request_id:retryId},failing:DocumentMailProvider={async sendDocument(){throw new Error('smtp offline');}};await assert.rejects(()=>sendDocumentEmail(seller,document.id,retryPayload,failing),{status:502});const failed=(await getDocument(seller,document.id)).emails.find(item=>item.status==='failed');assert.ok(failed);await sendDocumentEmail(seller,document.id,retryPayload,mail);assert.equal(sends,2);assert.equal((await getDocument(seller,document.id)).emails.filter(item=>item.status==='sent').length,2);
 const uncertainId=randomUUID(),uncertainPayload={recipient:'uncertain@example.com',subject:'Aceito pelo SMTP',message:'Teste de confirmação.',client_request_id:uncertainId};await database().query(`CREATE FUNCTION reject_email_confirmation_test() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.recipient='uncertain@example.com' AND NEW.status='sent' THEN RAISE EXCEPTION 'confirmation failed'; END IF; RETURN NEW; END $$; CREATE TRIGGER reject_email_confirmation_test BEFORE UPDATE ON crm_document_emails FOR EACH ROW EXECUTE FUNCTION reject_email_confirmation_test()`);await assert.rejects(()=>sendDocumentEmail(seller,document.id,uncertainPayload,mail));await assert.rejects(()=>sendDocumentEmail(seller,document.id,uncertainPayload,mail),{status:409});assert.equal(sends,3);await database().query('DROP TRIGGER reject_email_confirmation_test ON crm_document_emails; DROP FUNCTION reject_email_confirmation_test()');assert.equal((await database().query('SELECT status FROM crm_document_emails WHERE organization_id=$1 AND document_id=$2 AND client_request_id=$3',[seller.organizationId,document.id,uncertainId])).rows[0].status,'pending');
...
test('checklist, fotos, documentos, pendências, conclusão e aceite preservam autorização e histórico',async()=>{
 ...
 const storageProvider=process.env.DOCUMENT_STORAGE_PROVIDER;process.env.DOCUMENT_STORAGE_PROVIDER='invalid-test-provider';await deleteInstallationFile(admin,photo.id,photo.version);const pendingDeletion=(await database().query("SELECT id,status FROM storage_deletion_jobs WHERE organization_id=$1 AND source_type='installation_file' AND source_id=$2",[admin.organizationId,photo.id])).rows[0];assert.equal(pendingDeletion.status,'pending');process.env.DOCUMENT_STORAGE_PROVIDER=storageProvider;assert.deepEqual(await processStorageDeletionJobs(1,[pendingDeletion.id]),{processed:1,completed:1});await assert.rejects(()=>getInstallationFile(admin,photo.id),{status:404});
...
test('pós-venda vincula garantia, chamado, manutenção, arquivos e tarefa sem duplicar sistemas',async()=>{
 const client=await saveRecord(admin,'customer',{name:'Cliente Pós-venda Integrado',email:'posvenda-integrado@test.local'});
 const contract=await saveContract(admin,{client_id:client.id,title:'Sistema pós-venda',items:[{description:'Módulo garantido',category:'manual',quantity:1,unit_value:2500}],down_payment_value:2500,payment_method:'pix',installments_count:0});
 await setContractStatus(admin,contract.id,{status:'signed',version:contract.version});
 const installation=await saveInstallation(admin,{contract_id:contract.id,responsible_user_id:admin.userId,installation_address:'Rua Pós-venda, 123, Contagem - MG',team_name:'Equipe técnica',planned_on:'',scheduled_on:'',started_on:'',completed_on:'',notes:''});
 const item=(await getContractDetail(admin,contract.id)).items[0];
 const warrantyInput={installation_id:installation.id,contract_item_id:item.id,category:'module',description:'Garantia dos módulos',manufacturer:'Fabricante Solar',serial_number:'SN-001',supplier:'Fornecedor Solar',start_on:'2026-01-01',duration_months:12,notes:'Cobertura de fábrica.',lifecycle:'normal',responsible_user_id:admin.userId};
 await assert.rejects(()=>saveWarranty(admin,{...warrantyInput,lifecycle:'claimed'}),{status:400});
 let warranty=await saveWarranty(admin,warrantyInput);
 assert.equal(warranty.end_on,'2026-12-31');assert.equal((await listWarranties(admin,{installation_id:installation.id})).total,1);
 assert.equal((await getWarranty(admin,warranty.id)).client_id,client.id);
 await assert.rejects(()=>getWarranty(otherTenant,warranty.id),{status:404});
 await assert.rejects(()=>saveWarranty(admin,{...warrantyInput,contract_item_id:randomUUID()}),{status:400});
 warranty=await saveWarranty(admin,{...warrantyInput,description:'Garantia dos módulos e conectores',version:warranty.version},warranty.id);
 await assert.rejects(()=>saveWarranty(admin,{...warrantyInput,version:1},warranty.id),{status:409});
 const ticketInput={client_id:client.id,contract_id:contract.id,installation_id:installation.id,warranty_id:warranty.id,contract_item_id:item.id,title:'Módulo com falha',description:'Verificar geração.',category:'warranty',priority:'critical',status:'open',responsible_user_id:admin.userId,due_on:'2026-12-10',resolution:''};
 let ticket=await saveTicket(admin,ticketInput);
 assert.match(ticket.ticket_number,/^POS-\d{4}-\d{6}$/);assert.equal((await listTickets(admin,{q:ticket.ticket_number})).total,1);
 await assert.rejects(()=>getTicket(otherSeller,ticket.id),{status:404});await assert.rejects(()=>getTicket(otherTenant,ticket.id),{status:404});
 await addTicketComment(admin,ticket.id,'Cliente informou queda de geração.');
 ticket=await saveTicket(admin,{...ticketInput,status:'in_service',version:ticket.version},ticket.id);
 assert.ok(ticket.attended_at);
 await assert.rejects(()=>saveTicket(admin,{...ticketInput,status:'closed',resolution:'',version:ticket.version},ticket.id),{status:400});
 const claimInput={warranty_id:warranty.id,ticket_id:ticket.id,reason:'Módulo sem geração.',supplier:'Fornecedor Solar',protocol:'PROTO-1',notes:'',status:'awaiting_send',result:'',responsible_user_id:admin.userId};
 let claim=await saveClaim(admin,claimInput);assert.equal((await listClaims(admin,warranty.id)).length,1);
 assert.equal((await getWarranty(admin,warranty.id)).effective_status,'claimed');
 claim=await saveClaim(admin,{...claimInput,status:'sent',version:claim.version},claim.id);assert.equal(claim.status,'sent');
 await assert.rejects(()=>saveClaim(admin,{...claimInput,status:'closed',result:'',version:claim.version},claim.id),{status:400});
 claim=await saveClaim(admin,{...claimInput,status:'closed',result:'Módulo substituído.',version:claim.version},claim.id);assert.ok(claim.closed_at);
 const claimedWarranty=await getWarranty(admin,warranty.id);
 await assert.rejects(()=>saveWarranty(admin,{...warrantyInput,lifecycle:'normal',version:claimedWarranty.version},warranty.id),{status:409});
 const supportUser=(await database().query("INSERT INTO users(email,name,password_hash) VALUES ('post-sales-support@crm.test','Atendimento pós-venda',$1) RETURNING id",[await hashPassword(password)])).rows[0];
 await database().query("INSERT INTO memberships(organization_id,user_id,role_code) VALUES ($1,$2,'support')",[admin.organizationId,supportUser.id]);
 const support=(await sessionActor(await login({organization:'peclat-solar',email:'post-sales-support@crm.test',password})))!;
 const supportWarranty=await saveWarranty(admin,{...warrantyInput,description:'Garantia em atendimento',responsible_user_id:support.userId});
 const supportTicket=await saveTicket(admin,{...ticketInput,title:'Chamado atribuído ao atendimento',warranty_id:supportWarranty.id,responsible_user_id:support.userId});
 const supportClaim=await saveClaim(support,{...claimInput,warranty_id:supportWarranty.id,ticket_id:supportTicket.id,responsible_user_id:support.userId});
 await assert.rejects(()=>saveClaim(support,{...claimInput,warranty_id:supportWarranty.id,ticket_id:supportTicket.id,responsible_user_id:support.userId,status:'closed',result:'Não autorizado',version:supportClaim.version},supportClaim.id),{status:403});
 const supportOptions=await postSalesOptions(support);
 assert.ok(supportOptions.clients.some(row=>row.id===client.id));assert.ok(supportOptions.contracts.some(row=>row.id===contract.id));assert.ok(supportOptions.installations.some(row=>row.id===installation.id));assert.deepEqual(supportOptions.owners.map(row=>row.id),[support.userId]);
 const maintenanceInput={installation_id:installation.id,ticket_id:ticket.id,warranty_id:warranty.id,type:'corrective',reason:'Substituir módulo',description:'Intervenção em campo',responsible_user_id:admin.userId,planned_on:'2026-12-12',scheduled_on:'',executed_on:'',status:'requested',notes:'',resolution:'',internal_cost:180,charged_amount:0,no_charge:true,financial_notes:'Coberto pela garantia'};
 let maintenance=await saveMaintenance(admin,maintenanceInput);
 assert.equal((await listMaintenances(admin,{installation_id:installation.id})).total,1);
 await addMaintenanceItem(admin,{maintenance_id:maintenance.id,description:'Substituição de módulo',quantity:1,notes:'Peça em garantia'});
 assert.equal((await listMaintenanceItems(admin,maintenance.id)).length,1);
 maintenance=await saveMaintenance(admin,{...maintenanceInput,status:'scheduled',scheduled_on:'2026-12-12',version:maintenance.version},maintenance.id);
 maintenance=await saveMaintenance(admin,{...maintenanceInput,status:'completed',scheduled_on:'2026-12-12',executed_on:'2026-12-12',resolution:'Módulo trocado e testado.',version:maintenance.version},maintenance.id);
 assert.equal((await getMaintenance(admin,maintenance.id)).status,'completed');
 await assert.rejects(()=>addMaintenanceItem(admin,{maintenance_id:maintenance.id,description:'Item tardio',quantity:1}),{status:409});
 const bytes=Buffer.from('Relatório de garantia e manutenção');
 const file=await addPostSalesFile(admin,{parent_type:'ticket',parent_id:ticket.id,name:'Relatório técnico',description:'Anexo interno',category:'report'},{name:'relatorio.txt',type:'text/plain',bytes});
 assert.deepEqual(Buffer.from((await getPostSalesFile(admin,file.id)).bytes),bytes);
 assert.equal((await listPostSalesFiles(admin,'ticket',ticket.id)).length,1);
 await assert.rejects(()=>getPostSalesFile(otherTenant,file.id),{status:404});
 await assert.rejects(()=>addPostSalesFile(admin,{parent_type:'ticket',parent_id:ticket.id,name:'Falso',description:'',category:'other'},{name:'fake.png',type:'image/png',bytes:Buffer.from('falso')}),{status:415});
 await deletePostSalesFile(admin,file.id,file.version);assert.equal((await database().query("SELECT status FROM storage_deletion_jobs WHERE organization_id=$1 AND source_type='post_sales_file' AND source_id=$2",[admin.organizationId,file.id])).rows[0].status,'completed');await assert.rejects(()=>getPostSalesFile(admin,file.id),{status:404});
 const task=await saveTask(admin,{record_id:client.id,opportunity_id:null,title:'Retorno pós-venda',description:'Ligar para o cliente',priority:'normal',status:'pending',due_date:'2026-12-15',due_time:''});
 await linkTask(admin,{parent_type:'ticket',parent_id:ticket.id,task_id:task.id});
 const linkedTasks=await listLinkedTasks(admin,'ticket',ticket.id);
 assert.ok(Array.isArray(linkedTasks));assert.equal(linkedTasks.length,1);
 assert.ok(!(await postSalesOptions(otherSeller)).contractItems.some(row=>row.id===item.id));
 ticket=await saveTicket(admin,{...ticketInput,status:'resolved',resolution:'Módulo substituído e geração normalizada.',version:ticket.version},ticket.id);
 assert.ok(ticket.resolved_at);assert.ok((await ticketHistory(admin,ticket.id)).some(event=>event.action==='resolved'));
 const counts=await postSalesDashboard(admin);assert.ok(Number(counts.resolved_period)>=0);assert.ok(Number(counts.claimed_warranties)>=1);assert.ok(Number(counts.completed_maintenances)>=1);
});
