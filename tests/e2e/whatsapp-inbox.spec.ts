...
test('inbox mantém layout estável, rolagem inteligente, envio e vínculo responsivos',async({page})=>{
 ...
 await lateral.click();await expect(page.getByTestId('conversation-header').getByRole('heading',{name:'Contato lateral E2E'})).toBeVisible();await expect(page.getByRole('link',{name:'Ver Lead'})).toBeVisible();await expect(page.getByRole('button',{name:'Criar Lead',exact:true})).toHaveCount(0);
 ...
 await leadDialog.getByLabel('Nome *').fill('Juliana Tavares');
 ...
 await page.getByLabel('Selecionar modelo aprovado').selectOption({label:'peclat_recuperacao_lead_1 · pt_BR'});
 const nameParameter=page.getByLabel('Parâmetro da mensagem 1');await expect(nameParameter).toHaveValue('Juliana');await expect(nameParameter).toBeEditable();
 await expect(page.locator('.whatsapp-template-preview')).toContainText('Olá, Juliana! Tudo bem? 😊');
 let templatePosts=0,templatePayload:Record<string,unknown>|null=null;
 await page.route('**/api/whatsapp/conversations/*/template',async route=>{templatePosts++;templatePayload=route.request().postDataJSON();await new Promise(resolve=>setTimeout(resolve,250));await route.continue();});
 await page.getByRole('button',{name:'Enviar modelo'}).dblclick();
 ...
 expect(templatePosts).toBe(1);expect(templatePayload).toMatchObject({parameters:{header:[],body:['Juliana']}});
 await expect(history.getByText('Olá, Juliana! Tudo bem? 😊',{exact:true})).toBeVisible();
 await page.getByLabel('Selecionar modelo aprovado').selectOption({label:'peclat_recuperacao_lead_1 · pt_BR'});await nameParameter.fill('Falhar Meta');await page.getByRole('button',{name:'Enviar modelo'}).click();await expect(page.locator('.error-box[role="alert"]')).toContainText('Envio recusado pela Meta: Falha simulada da Meta');
 await page.unroute('**/api/whatsapp/conversations/*/template');
 await page.reload();
 await page.getByLabel('Buscar conversas').fill('5531982223344');
 await page.getByRole('button',{name:'Aplicar filtros'}).click();
 await page.getByRole('button',{name:/Juliana Tavares/}).click();
 await expect(page.getByRole('link',{name:'Ver Lead'})).toBeVisible();
 ...
 await expect(page.getByRole('heading',{name:'Juliana Tavares'})).toBeVisible();
 ...
 await expect(page.getByText(/Olá, Juliana! Tudo bem/)).toBeVisible();
 ...
 await inboxPage.getByRole('button',{name:/Juliana Tavares/}).click();
