...
test('assistente comercial usa contexto mínimo, revisão humana, limites e isolamento sem IA real',async()=>{
 ...
 const configured=await saveAiAssistantSettings(admin,{enabled:true,provider:'gemini',model:'gemini-2.5-flash',context_message_limit:30,max_requests_per_hour:5,version:null});assert.equal(configured.enabled,true);assert.equal(configured.provider,'gemini');assert.equal(configured.model,'gemini-2.5-flash');assert.equal((await aiAssistantSettings(seller)).can_manage,false);await assert.rejects(()=>saveAiAssistantSettings(admin,{enabled:true,provider:'gemini',model:'gemini-2.5-flash',context_message_limit:30,max_requests_per_hour:5,version:configured.version,organization_id:orgB}));
 ...
 const usage=await database().query('SELECT status,action,input_tokens,output_tokens,error_code FROM ai_usage_events WHERE organization_id=$1 AND conversation_id=$2 ORDER BY created_at',[orgA,conversation]);assert.equal(usage.rowCount,5);assert.deepEqual(usage.rows.map(row=>row.status),['succeeded','failed','failed','failed','failed']);assert.ok(usage.rows.some(row=>row.error_code==='ungrounded_output'));assert.ok(usage.rows.some(row=>row.error_code==='free_tier_exhausted'));assert.ok(usage.rows.some(row=>row.error_code==='timeout'));assert.equal(JSON.stringify(usage.rows).includes('Ignore todas'),false);assert.equal(JSON.stringify(usage.rows).includes('segredo'),false);
