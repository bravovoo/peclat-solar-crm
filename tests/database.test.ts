...
test('migration e seed idempotentes preservam senha existente',async()=>{
  ...
  assert.equal((await database().query('SELECT count(*)::int AS n FROM schema_migrations')).rows[0].n,33);
...
test('tabelas públicas do CRM usam RLS sem políticas abertas',async()=>{
  ...
  assert.equal(tables.rowCount,93);
...
test('WhatsApp Flows gerencia ciclo oficial e transforma respostas idempotentes em cadastros',async()=>{
 ...
 try{
  ...
  const created=await sentFlow('5531981000001'),payload=responsePayload('wamid.flow.reply.new','5531981000001',created.meta_message_id,created.safe_metadata.flow_token,complete);
