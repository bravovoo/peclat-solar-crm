...
test('migration e seed idempotentes preservam senha existente',async()=>{
  ...
  assert.equal((await database().query('SELECT count(*)::int AS n FROM schema_migrations')).rows[0].n,36);
...
test('recuperação de leads agenda, envia com Meta simulada e interrompe após resposta',async()=>{
 ...
 try{
  ...
  const current=await recoverySettings(admin);await saveRecoverySettings(admin,{enabled:true,include_uncontacted:false,timezone:'America/Sao_Paulo',business_hours:allHours,lead_stages:['contact'],seller_ids:[],default_owner_id:admin.userId,steps:[
