...
import {getWhatsAppConversation,linkWhatsAppConversation,listWhatsAppConversations,markWhatsAppConversationRead,whatsappConversationsForRecord} from '../src/modules/whatsapp/inbox';
import {listWhatsAppTemplates,sendWhatsAppTemplate,sendWhatsAppText,syncWhatsAppTemplates} from '../src/modules/whatsapp/outbound';
...
test('migration e seed idempotentes preservam senha existente',async()=>{
  ...
  assert.equal((await database().query('SELECT count(*)::int AS n FROM schema_migrations')).rows[0].n,20);
});
...
test('tabelas públicas do CRM usam RLS sem políticas abertas',async()=>{
  ...
  assert.equal(tables.rowCount,68);
