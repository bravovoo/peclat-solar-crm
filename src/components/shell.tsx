...
const futureItems = [ ['WhatsApp',MessageCircle],['Propostas',FileText],['Pós-venda',HeartHandshake],['Relatórios',ChartNoAxesCombined],['Automação',Workflow] ] as const;
export function Shell({ actor, children }: {actor: Actor; children: React.ReactNode}) {
  const router=useRouter();
  const sidebarRef=useRef<HTMLElement>(null);
  const menuRef=useRef<HTMLButtonElement>(null);
  const [open,setOpen]=useState(false); const [busy,setBusy]=useState(false); const [error,setError]=useState('');
  const path=usePathname(); const commercial=actor.permissions.some(p=>p==='crm.all'||p==='crm.own'); const contracts=actor.permissions.some(p=>p==='contracts.all'||p==='contracts.own'); const installations=actor.permissions.includes('installations.read'); const section=path.split('/')[1]; const title=({equipe:'Equipe',configuracoes:'Configurações',leads:'Leads',clientes:'Clientes',empresas:'Empresas',tags:'Tags',pipeline:'Pipeline',oportunidades:'Oportunidades',tarefas:'Tarefas',agenda:'Agenda','follow-up':'Follow-up','catalogo-solar':'Catálogo solar',contratos:'Contratos',instalacoes:'Instalações'} as Record<string,string>)[section]??'Dashboard';
  useEffect(()=>{
    if(!open)return;
