...
export function OpportunityDetail({initial,owners,userId,whatsappAvailable=false}:{initial:Opportunity;owners:Owners;userId:string;whatsappAvailable?:boolean}){
 ...
 const tabs={summary:'Resumo',activities:'Histórico e atividades',notes:'Notas',tasks:'Tarefas',documents:'Documentos',contracts:'Contratos'};
 return <><Link className="back-link" href="/pipeline">← Voltar ao pipeline</Link><div className="page-heading"><div><span className="eyebrow blue">OPERAÇÃO COMERCIAL</span><h1>{o.title}</h1><div className="detail-meta"><span className="badge">{opportunityStages[o.stage]}</span><span>{o.owner_name}</span><strong>{money(o.estimated_value)}</strong></div></div><div className="crm-form-actions">{whatsappAvailable&&<Link className="button secondary" href="/whatsapp"><MessageCircle size={16}/>WhatsApp</Link>}<button className="button primary" onClick={()=>setEditing(true)}>Editar oportunidade</button></div></div>{notice&&<p role="status" className="crm-notice">{notice}</p>}{error&&<p role="alert" className="error-box">{error}</p>}
 ...
 <section className="card detail-content">
  ...
  {tab==='contracts'&&<RelatedContracts opportunityId={o.id}/>}
