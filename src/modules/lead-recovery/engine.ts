...
export async function handleLeadRecoveryInbound(db:Db,input:{organizationId:string;conversationId:string;recordId:string|null;actorId:string;timestamp:Date;text:string;messageId?:string}){
...
async function persistWhatsAppOptOut(db:Db,input:{organizationId:string;conversationId:string;timestamp:Date;messageId?:string},recordId:string|null,actorId:string){
 if(recordId){
  await db.query(`INSERT INTO crm_contact_preferences(organization_id,record_id,whatsapp_consent_status,consent_source,opted_out_at,updated_by) VALUES ($1,$2,'opted_out','Solicitação recebida pelo WhatsApp',$4,$3) ON CONFLICT(organization_id,record_id) DO UPDATE SET whatsapp_consent_status='opted_out',opted_out_at=EXCLUDED.opted_out_at,updated_by=EXCLUDED.updated_by,version=crm_contact_preferences.version+1,updated_at=now()`,[input.organizationId,recordId,actorId,input.timestamp]);
  const contact=await db.query<{contact_id:string}>(`SELECT contact_id FROM crm_record_contacts WHERE organization_id=$1 AND record_id=$2 ORDER BY is_primary DESC,contact_id LIMIT 1`,[input.organizationId,recordId]);
  await db.query(`INSERT INTO audit_logs(organization_id,actor_id,action,detail) VALUES ($1,$2,'whatsapp.marketing_opt_out_verified',$3)`,[input.organizationId,actorId,`Opt-out WhatsApp; lead_id=${recordId}; contact_id=${contact.rows[0]?.contact_id??'sem_contato_cadastrado'}; conversation_id=${input.conversationId}; meta_message_id=${input.messageId??'indisponivel'}; data=${input.timestamp.toISOString()}.`]);
 }
