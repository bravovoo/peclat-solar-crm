...
import {nextRecoveryWindow,recoveryIsOpen,recoveryEligibleAt,recoveryWindowEnd,recoveryWindowStart} from './schedule';
...
const defaultHours=businessHoursSchema.parse(Object.fromEntries(['1','2','3','4','5','6','7'].map(day=>[day,{enabled:day!=='7',start:recoveryWindowStart,end:recoveryWindowEnd}])));
...
export async function saveRecoverySettings(actor:Actor,input:unknown){
 ...
 return transaction(async db=>{
  ...
  await db.query(`UPDATE lead_recovery_enrollments SET sequence_snapshot=jsonb_set(jsonb_set(sequence_snapshot,'{business_hours}',$2::jsonb,true),'{timezone}',to_jsonb($3::text),true),updated_at=now() WHERE organization_id=$1 AND status IN ('scheduled','paused')`,[actor.organizationId,JSON.stringify(data.business_hours),data.timezone]);
  const pending=await db.query<{id:string;enrollment_id:string;eligible_at:string|null;inactivity_anchor:string;day_offset:number|null}>(`SELECT attempt.id,attempt.enrollment_id,attempt.eligible_at,enrollment.inactivity_anchor,attempt.day_offset FROM lead_recovery_attempts attempt JOIN lead_recovery_enrollments enrollment ON enrollment.organization_id=attempt.organization_id AND enrollment.id=attempt.enrollment_id WHERE attempt.organization_id=$1 AND attempt.status='pending' AND enrollment.status='scheduled' AND enrollment.state_reason<>'reschedule' FOR UPDATE OF attempt,enrollment`,[actor.organizationId]);
  for(const attempt of pending.rows){if(!attempt.day_offset)continue;const eligible=attempt.eligible_at?new Date(attempt.eligible_at):recoveryEligibleAt(attempt.inactivity_anchor,attempt.day_offset),scheduled=nextRecoveryWindow(eligible,data.business_hours);await db.query("UPDATE lead_recovery_attempts SET eligible_at=$3,scheduled_for=$4,safe_error=CASE WHEN safe_error='outside_business_hours' THEN '' ELSE safe_error END,updated_at=now() WHERE organization_id=$1 AND id=$2",[actor.organizationId,attempt.id,eligible,scheduled]);await db.query('UPDATE lead_recovery_enrollments SET next_attempt_at=$3,updated_at=now() WHERE organization_id=$1 AND id=$2',[actor.organizationId,attempt.enrollment_id,scheduled]);}
...
export async function recoveryDashboard(actor:Actor,input:unknown={}){
 ...
 const filtered=applyFilters(items,filters),offset=(filters.page-1)*filters.page_size,metrics={eligible:items.filter(item=>item.eligible).length,scheduled:items.filter(item=>item.enrollment_status==='scheduled').length,sent:0,responded:items.filter(item=>item.enrollment_status==='responded').length,unanswered:items.filter(item=>item.enrollment_status==='scheduled'&&Number(item.attempt_count)>0).length,suspended:items.filter(item=>item.enrollment_status==='paused'||['opted_out','contact_blocked','conversation_paused'].includes(item.reason)).length,errors:items.filter(item=>item.enrollment_status==='error').length};
 const sent=await database().query<{total:number}>(`SELECT count(*)::int total FROM lead_recovery_attempts a JOIN lead_recovery_enrollments e ON e.organization_id=a.organization_id AND e.id=a.enrollment_id JOIN crm_records r ON r.organization_id=e.organization_id AND r.id=e.record_id WHERE ${commercialScope(actor,'r',true)} AND r.kind='lead' AND r.deleted_at IS NULL AND a.status='sent'`,commercialScopeParams(actor));metrics.sent=Number(sent.rows[0].total);
...
export async function operateRecovery(actor:Actor,id:string,input:unknown){operate(actor);const data=recoveryOperationInput.parse(input);return transaction(async db=>{await db.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',[actor.organizationId]);const current=await scopedEnrollment(db,actor,id,true);if(Number(current.version)!==data.version)throw new AccessError(409,'O acompanhamento foi atualizado. Recarregue a página.');if(!['scheduled','paused'].includes(current.status))throw new AccessError(409,'Este ciclo já foi encerrado. Uma nova mensagem da empresa poderá iniciar outro ciclo.');if(['resume','reschedule'].includes(data.action)){const attempt=await db.query<{status:string}>("SELECT status FROM lead_recovery_attempts WHERE organization_id=$1 AND enrollment_id=$2 AND step_position=$3 FOR UPDATE",[actor.organizationId,id,Number(current.attempt_count)+1]);if(attempt.rows[0]?.status==='processing')throw new AccessError(409,'Uma tentativa está em processamento. Aguarde a conclusão antes de reagendar.');}let status=current.status,next=current.next_attempt_at;const reason=data.action;if(data.action==='pause')status='paused';else if(data.action==='cancel')status='cancelled';else if(data.action==='resume'){status='scheduled';next=new Date().toISOString();}else{status='scheduled';next=data.scheduled_for;}
 ...
 if(status==='scheduled')await db.query("UPDATE lead_recovery_attempts SET status='pending',scheduled_for=$3,safe_error='',completed_at=NULL,updated_at=now() WHERE organization_id=$1 AND enrollment_id=$2 AND status IN ('cancelled','pending') AND step_position=$4",[actor.organizationId,id,next,Number(current.attempt_count)+1]);
