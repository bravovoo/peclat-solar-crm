...
export async function deletePostSalesFile(actor:Actor,id:string,version:unknown){requirePermission(actor,'post_sales.manage');const expected=Number(version);if(!Number.isInteger(expected)||expected<1)throw new AccessError(400,'Versão inválida.');const jobId=await transaction(async db=>{const {file,type,parentId,related}=await fileRow(actor,id,db,true);if(file.version!==expected)throw new AccessError(409,'Anexo atualizado por outra pessoa. Recarregue.');await db.query('UPDATE post_sales_files SET deleted_at=now(),deleted_by=$3,version=version+1,updated_at=now() WHERE organization_id=$1 AND id=$2',[actor.organizationId,id,actor.userId]);await history(db,actor,type,parentId,related.clientId,'file_removed',`${related.number} · ${file.name} removido.`);return enqueueStorageDeletion(db,actor.organizationId,'post_sales_file',file.id,file.storage_key);});await processStorageDeletionJobs(1,[jobId]);return {removed:true};}
