...
function scope(actor:Actor,alias='r'){crmAccess(actor);return `${alias}.organization_id=$1 AND ${alias}.deleted_at IS NULL`+(actor.permissions.includes('crm.all')?'':` AND ${alias}.owner_id=$2`);}
function scopeParams(actor:Actor):unknown[]{return actor.permissions.includes('crm.all')?[actor.organizationId]:[actor.organizationId,actor.userId];}
...
const joins='FROM crm_records r JOIN users u ON u.id=r.owner_id';
...
export async function listRecords(actor:Actor,kind:Kind,input:unknown){
 ...
 return {records:result.rows.map(normalize),total:count.rows[0].total as number,page:f.page,pageSize:f.pageSize};
}
...
async function ownerAllowed(actor:Actor,owner:string,db:Db){
 if(!actor.permissions.includes('crm.all')&&owner!==actor.userId)throw new AccessError(403,'Você só pode atribuir cadastros a si mesmo.');
 const result=await db.query(`SELECT 1 FROM memberships m JOIN users u ON u.id=m.user_id WHERE m.organization_id=$1 AND m.user_id=$2 AND m.active AND u.active AND EXISTS(SELECT 1 FROM role_permissions rp WHERE rp.role_code=m.role_code AND rp.permission_code IN ('crm.all','crm.own'))`,[actor.organizationId,owner]);
 if(!result.rowCount)throw new AccessError(400,'Responsável inválido para esta organização.');
}
...
async function duplicates(db:Db,actor:Actor,data:RecordData,exclude?:CommercialRecord){
 ...
 if(rows.length){const canOverride=actor.permissions.includes('crm.duplicate.override');if(!data.allow_duplicate||!canOverride)throw new DuplicateError(rows.filter(r=>actor.permissions.includes('crm.all')||r.owner_id===actor.userId).map(({id,kind,name})=>({id,kind,name})),canOverride);}
}
...
export async function saveRecord(actor:Actor,kind:Kind,input:unknown,id?:string){
 ...
 return transaction(async db=>{
  ...
  const owner=data.owner_id??before?.owner_id??actor.userId;await ownerAllowed(actor,owner,db);
  ...
  if(before){
   ...
   if(before.owner_id!==owner)await activity(db,actor,id!,'owner.changed','Responsável alterado.');
...
export async function crmOptions(actor:Actor){
 crmAccess(actor);
 const owners=await database().query(`SELECT u.id,u.name FROM memberships m JOIN users u ON u.id=m.user_id WHERE m.organization_id=$1 AND m.active AND u.active AND EXISTS(SELECT 1 FROM role_permissions rp WHERE rp.role_code=m.role_code AND rp.permission_code IN ('crm.all','crm.own')) ${actor.permissions.includes('crm.all')?'':'AND u.id=$2'} ORDER BY u.name LIMIT 200`,scopeParams(actor));
 ...
 return {owners:owners.rows as {id:string;name:string}[],tags:tags.rows};
}
