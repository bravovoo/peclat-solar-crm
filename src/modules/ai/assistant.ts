...
type Setting={enabled:boolean;provider:string;model:string;context_message_limit:number;max_requests_per_hour:number;version:number;updated_at:string};
const defaults={enabled:false,provider:'openai',model:'gpt-5-mini',context_message_limit:40,max_requests_per_hour:60,version:null as number|null,updated_at:null as string|null};
export async function aiAssistantSettings(actor:Actor){
 ...
 return {...(row??defaults),configured:aiProviderConfigured(),can_manage:actor.permissions.includes('ai_assistant.manage')};
}
...
function messageContent(row:{message_type:string;text_body:string;caption:string;filename:string}){
 if(row.message_type==='text'||row.message_type==='template')return row.text_body.slice(0,4096);
 ...
 return `${labels[row.message_type]??'Conteúdo recebido'}${row.caption?`: ${row.caption.slice(0,1000)}`:''}${row.message_type==='document'&&row.filename?` (${row.filename.slice(0,180)})`:''}`;
}
...
async function reserveUsage(actor:Actor,conversationId:string,requestId:string,action:string,setting:Setting){
...
export async function runCommercialAi(actor:Actor,input:unknown,provider?:CommercialAiProvider){
 ...
 await assertWhatsAppConversation(actor,data.conversation_id);const eventId=await reserveUsage(actor,data.conversation_id,data.request_id,data.action,setting),started=Date.now();
 try{const context=await buildCommercialAiContext(actor,data.conversation_id,setting.context_message_limit),result=await (provider??configuredAiProvider(setting.model)).generate({action:data.action,context,signal:AbortSignal.timeout(15000)});assertGrounded(result.output,context);
