...
const defaults={enabled:false,provider:'gemini',model:'gemini-2.5-flash',context_message_limit:40,max_requests_per_hour:60,version:null as number|null,updated_at:null as string|null};
...
export async function runCommercialAi(actor:Actor,input:unknown,provider?:CommercialAiProvider){
 ...
 try{const context=await buildCommercialAiContext(actor,data.conversation_id,setting.context_message_limit),result=await (provider??configuredAiProvider(setting.provider,setting.model)).generate({action:data.action,context,signal:AbortSignal.timeout(15000)});assertGrounded(result.output,context);
 ...
 }catch(error){const code=error instanceof AiProviderError?error.code:error instanceof DOMException&&error.name==='TimeoutError'?'timeout':'provider_failure';await database().query("UPDATE ai_usage_events SET status='failed',duration_ms=$3,error_code=$4,finished_at=now() WHERE organization_id=$1 AND id=$2",[actor.organizationId,eventId,Date.now()-started,code]);if(error instanceof AccessError)throw error;if(code==='free_tier_exhausted')throw new AccessError(429,'Limite gratuito do Gemini atingido. Aguarde a renovação da cota ou tente novamente mais tarde.');if(code==='rate_limited')throw new AccessError(429,'O assistente está temporariamente ocupado. Tente novamente.');throw new AccessError(503,'Não foi possível gerar a sugestão agora. Tente novamente.');}
}
